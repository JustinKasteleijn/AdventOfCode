...
namespace AdventOfCode {
    int DayThirteen::parse(const std::vector<std::string> &data, bool part2) {
        ...
        std::transform(happiness.begin(), happiness.end(), people.begin(),
                       [](const std::pair<const std::string, std::map<std::string, int>> &p) { return p.first; });

        int max_happiness = 0;
        do {
            int current_happiness = !part2 ? (happiness[*(people.end() - 1)][*people.begin()] +
                                              happiness[*people.begin()][*(people.end() - 1)]) : 0;
